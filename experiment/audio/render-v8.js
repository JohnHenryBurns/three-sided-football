...
function renderCry(D=2.6,seed=5){
  ...
  for(let n=0;n<N;n++){
    ...
    if((n&63)===0){
      let F;
      if(frac<0.48) F=VOW.oh;
      else if(frac<0.74) F=VOW.oh.map((v,k)=>lerp(v,VOW.ah[k],(frac-0.48)/0.26));
      else if(frac<0.84) F=VOW.ah.map((v,k)=>lerp(v,VOW.l[k],(frac-0.74)/0.10));
      else F=VOW.l;                              // SIT on the L — this is the enunciation
