...
const VOW={oh:[520,880,2600],ah:[790,1240,2700],l:[360,1020,3000]};
...
function renderCry(D=2.6,seed=5){
  ...
  const N=Math.floor(SR*(D*1.30)), out=new Float32Array(N);

  // pitch: fast onset rise, then HOLD (not a glide), quick fall at the end
  const pitchPts=[[0,208],[0.10*D,242],[0.22*D,250],[0.62*D,254],[0.80*D,246],[0.97*D,230],[1.08*D,200],[1.26*D,176]];
  // amplitude: hard attack, sustained plateau, short release
  const ampPts=[[0,0],[0.012,1.0],[0.10*D,0.98],[0.50*D,1.0],[0.70*D,1.04],
                [0.78*D,0.98],[0.82*D,0.86],[0.97*D,0.80],[1.02*D,0.52],
                [1.07*D,0.30],[1.13*D,0.14],[1.19*D,0.05],[1.26*D,0.0]];
  const vibPts=[[0,0.0008],[0.35*D,0.0012],[0.75*D,0.0022],[D,0.0018]]; // essentially steady
  ...
  const hp=mkF(),lp=mkF(), cLp=mkF(), cHp=mkF(), gF=mkF(), lF=mkF();
  const coLdark=lowpass(2100,0.7,SR);
  ...
  for(let n=0;n<N;n++){
    ...
    if((n&63)===0){
      let F;
      const GT=0.055;                            // the /g/ release transition
      if(frac<GT){
        const u=frac/GT;                         // F2 and F3 pinched together, then separating
        F=[lerp(260,VOW.oh[0],u), lerp(1950,VOW.oh[1],u), lerp(2250,VOW.oh[2],u)];
      }
      else if(frac<0.44) F=VOW.oh;               // STEADY oh
      else if(frac<0.52) F=VOW.oh.map((v,k)=>lerp(v,VOW.ah[k],(frac-0.44)/0.08));  // fast move
      else if(frac<0.78) F=VOW.ah;               // STEADY aah
      else if(frac<0.85) F=VOW.ah.map((v,k)=>lerp(v,VOW.l[k],(frac-0.78)/0.07));   // fast move
      else F=VOW.l;                              // STEADY L
    ...
    if(--shimCd<=0){ shimTarget=0.985+rnd()*0.030; shimCd=Math.floor(SR*(0.05+rnd()*0.05)); }
    shim+=(shimTarget-shim)*0.0015;              // a shout is STEADY — tremolo is a bleat
    ...
    if(frac>0.80){ v=run(lF,coLdark,v)*1.05; }   // lateral: the tongue closes, the top rolls off

    // the "G" attack burst
    if(t<0.016) v*=Math.pow(t/0.016,0.55);      // voicing bursts out of the closure
  ...
  for(let n=0;n<N;n++) out[n]=Math.tanh(out[n]*2.4)/Math.tanh(2.4);
  ...
  const GL=Math.floor(0.012*SR);               // a stop burst is SHORT and hard
  for(let n=0;n<GL;n++){
    const k=1-n/GL;
    out[n]=clamp(out[n]*0.94+run(gS,gC,(gr()*2-1))*1.9*k,-1,1);
  }
  return out;
}
