...
function renderCry(D=2.6,seed=5){
  ...
  const coHp=highpass(95,0.7,SR), coLp=lowpass(3300,0.8,SR);
  const coSrc=lowpass(1500,0.55,SR);        // glottal tilt: -12 dB/oct like a real voice
  const srcF=mkF();
  ...
  for(let n=0;n<N;n++){
    ...
    // the "G" attack burst
    if(t<0.040){ const k=1-t/0.040; v+=(rnd()*2-1)*0.30*k*k; }
    ...
    out[n]=clamp(v*seg(t,ampPts)*shim*0.66 + cr*seg(t,crowdPts)*0.15,-1,1);
  }
  ...
  for(let n=0;n<N;n++) out[n]=Math.tanh(out[n]*2.1)/Math.tanh(2.1);
  peak=0; for(let n=0;n<N;n++) peak=Math.max(peak,Math.abs(out[n]));
  if(peak>0){const g=0.95/peak; for(let n=0;n<N;n++) out[n]*=g;}
  return out;
}
